import { useEffect, useMemo, useState } from "react";
import { useSearchParams } from "react-router-dom";
import { Search, Send, User, Loader2, ArrowLeft } from "lucide-react";
import Avatar from "@/components/ui/Avatar";
import { useTranslation } from "@/hooks/useTranslation";
import {
  useConversations,
  useConversationMessages,
  useUpdateConversationStatus,
  useBlockConversationCustomer,
  useUnblockConversationCustomer,
  useSendMessage,
} from "@/hooks/useApi";
import toast from "react-hot-toast";

export default function MessagesPage() {
  const { t } = useTranslation();
  const [searchParams] = useSearchParams();
  const conversationFromQuery =
    Number(searchParams.get("conversation")) || null;
  const MAX_MESSAGE_LENGTH = 2000;
  const {
    data: conversationsRes,
    isLoading: convsLoading,
    isError: convsError,
  } = useConversations(true);
  const conversations = conversationsRes?.data ?? conversationsRes ?? [];
  const [selectedChatId, setSelectedChatId] = useState(null);
  const [isMobileChatOpen, setIsMobileChatOpen] = useState(false);
  const [messageText, setMessageText] = useState("");
  const [searchTerm, setSearchTerm] = useState("");
  const sendMessageMutation = useSendMessage();
  const updateStatusMutation = useUpdateConversationStatus();
  const blockCustomerMutation = useBlockConversationCustomer();
  const unblockCustomerMutation = useUnblockConversationCustomer();

  const { data: messagesRes, isLoading: msgsLoading } = useConversationMessages(
    selectedChatId,
    !!selectedChatId,
  );
  const selectedMessages =
    messagesRes?.data?.messages ?? messagesRes?.messages ?? [];
  const selectedConversationMeta = messagesRes?.data ?? {};

  // Polling for new messages is handled by React Query refetchInterval in useConversationMessages

  useEffect(() => {
    if (!conversations.length) return;

    if (conversationFromQuery) {
      const existing = conversations.find(
        (c) => Number(c.id) === conversationFromQuery,
      );
      if (existing) {
        setSelectedChatId(existing.id);
        setIsMobileChatOpen(true);
        return;
      }
    }

    if (!selectedChatId) {
      setSelectedChatId(conversations[0].id);
    }
  }, [conversations, selectedChatId, conversationFromQuery]);

  const selectedChat = useMemo(
    () => conversations.find((c) => c.id === selectedChatId) ?? null,
    [conversations, selectedChatId],
  );

  const visibleConversations = useMemo(
    () =>
      conversations.filter((c) =>
        c.name.toLowerCase().includes(searchTerm.toLowerCase()),
      ),
    [conversations, searchTerm],
  );

  const sendMessage = (e) => {
    e.preventDefault();
    const trimmedMessage = messageText.trim();
    if (!selectedChat || !trimmedMessage) return;
    if (trimmedMessage.length > MAX_MESSAGE_LENGTH) return;

    sendMessageMutation.mutate({
      conversation_id: selectedChat.id,
      message: trimmedMessage,
    });
    setMessageText("");
  };

  const setStatus = async (status) => {
    if (!selectedChat?.id) return;
    try {
      await updateStatusMutation.mutateAsync({
        conversationId: selectedChat.id,
        data: { status },
      });
      toast.success(t("merchant.conversationUpdated"));
    } catch (err) {
      toast.error(
        err?.response?.data?.message || t("merchant.failedUpdateStatus"),
      );
    }
  };

  const toggleBlock = async () => {
    if (!selectedChat?.id) return;
    const isBlocked =
      selectedChat?.is_customer_blocked ||
      selectedConversationMeta?.is_customer_blocked;
    try {
      if (isBlocked) {
        await unblockCustomerMutation.mutateAsync(selectedChat.id);
        toast.success(t("merchant.customerUnblocked"));
      } else {
        await blockCustomerMutation.mutateAsync(selectedChat.id);
        toast.success(t("merchant.customerBlocked"));
      }
    } catch (err) {
      toast.error(
        err?.response?.data?.message || t("merchant.failedUpdateBlock"),
      );
    }
  };

  return (
    <div className="flex h-[calc(100vh-8rem)] bg-white rounded-xl border border-border/50 overflow-hidden">
      {/* Conversations List */}
      <div
        className={`w-full sm:w-80 border-r border-border/50 flex flex-col ${isMobileChatOpen ? "hidden sm:flex" : "flex"}`}
      >
        <div className="p-4 border-b border-border/50">
          <h2 className="font-semibold text-text mb-3">
            {t("merchant.messages")}
          </h2>
          <div className="relative">
            <Search className="absolute start-3 top-1/2 -translate-y-1/2 h-4 w-4 text-text-secondary" />
            <input
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              placeholder={t("merchant.searchConversations")}
              className="w-full ps-9 pe-4 py-2 border border-border rounded-lg text-sm outline-none focus:border-primary"
            />
          </div>
        </div>
        <div className="flex-1 overflow-y-auto">
          {convsLoading && (
            <div className="flex items-center justify-center py-12">
              <Loader2 className="h-6 w-6 animate-spin text-primary" />
            </div>
          )}
          {convsError && (
            <div className="text-center py-12 text-sm text-danger">
              {t("merchant.failedLoadConversations")}
            </div>
          )}
          {!convsLoading && conversations.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-text-secondary p-6">
              <User className="h-8 w-8 mb-2" />
              <p className="text-sm font-medium">
                {t("merchant.noConversationsYet")}
              </p>
              <p className="text-xs">{t("merchant.messagesWillAppearHere")}</p>
            </div>
          )}
          {!convsLoading &&
            !convsError &&
            visibleConversations.map((conv) => (
              <button
                key={conv.id}
                onClick={() => {
                  setSelectedChatId(conv.id);
                  setIsMobileChatOpen(true);
                }}
                className={`w-full p-4 flex items-center gap-3 hover:bg-gray-50 transition border-b border-border/30 ${selectedChat?.id === conv.id ? "bg-primary/5 border-l-2 border-l-primary" : ""}`}
              >
                <div className="relative">
                  <Avatar name={conv.name} size="md" />
                  {conv.online && (
                    <span className="absolute bottom-0 right-0 w-3 h-3 bg-success rounded-full border-2 border-white" />
                  )}
                </div>
                <div className="flex-1 min-w-0 text-left">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-text">
                      {conv.name}
                    </span>
                    <span className="text-xs text-text-secondary">
                      {conv.time}
                    </span>
                  </div>
                  <p className="text-xs text-text-secondary truncate">
                    {conv.lastMessage}
                  </p>
                </div>
                {conv.unread > 0 && (
                  <span className="w-5 h-5 bg-primary text-white text-xs rounded-full flex items-center justify-center">
                    {conv.unread}
                  </span>
                )}
              </button>
            ))}
        </div>
      </div>

      {/* Chat Area */}
      {selectedChat ? (
        <div
          className={`flex-1 flex flex-col ${isMobileChatOpen ? "flex" : "hidden sm:flex"}`}
        >
          <div className="p-4 border-b border-border/50 flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-3">
              <button
                type="button"
                className="sm:hidden p-1 rounded hover:bg-gray-100"
                onClick={() => setIsMobileChatOpen(false)}
                aria-label={t("common.back") || "Back"}
              >
                <ArrowLeft className="h-5 w-5 text-text-secondary" />
              </button>
              <Avatar name={selectedChat.name} size="md" />
              <div>
                <p className="font-medium text-text">{selectedChat.name}</p>
                <p className="text-xs text-success">
                  {selectedChat.online
                    ? t("merchant.online")
                    : t("merchant.offline")}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2 gorw-1 justify-end">
              <button
                type="button"
                className="text-xs px-2 py-1 rounded bg-gray-100"
                onClick={() => setStatus("waiting")}
                disabled={updateStatusMutation.isPending}
              >
                {t("admin.waiting") || "Waiting"}
              </button>
              <button
                type="button"
                className="text-xs px-2 py-1 rounded bg-gray-100"
                onClick={() => setStatus("resolved")}
                disabled={updateStatusMutation.isPending}
              >
                {t("admin.resolve") || "Resolve"}
              </button>
              <button
                type="button"
                className={`text-xs px-2 py-1 rounded ${selectedChat?.is_customer_blocked || selectedConversationMeta?.is_customer_blocked ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                onClick={toggleBlock}
                disabled={
                  blockCustomerMutation.isPending ||
                  unblockCustomerMutation.isPending
                }
              >
                {selectedChat?.is_customer_blocked ||
                selectedConversationMeta?.is_customer_blocked
                  ? t("admin.unblock") || "Unblock"
                  : t("admin.block") || "Block"}
              </button>
            </div>
          </div>
          <div
            className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50"
            role="log"
            aria-label="Messages"
          >
            {msgsLoading && (
              <div className="flex items-center justify-center py-12">
                <Loader2 className="h-6 w-6 animate-spin text-primary" />
              </div>
            )}
            {!msgsLoading &&
              selectedMessages.map((msg) => (
                <div
                  key={msg.id}
                  className={`flex ${msg.sender === "merchant" ? "justify-end" : "justify-start"}`}
                >
                  <div
                    className={`max-w-[70%] px-4 py-2.5 rounded-2xl ${msg.sender === "merchant" ? "bg-primary text-white rounded-br-md" : msg.sender === "admin" ? "bg-secondary text-white rounded-bl-md" : "bg-white border border-border/50 text-text rounded-bl-md"}`}
                  >
                    {msg.sender === "admin" && (
                      <p className="text-[11px] font-semibold text-white/80 mb-0.5">
                        {msg.sender_name ||
                          `${import.meta.env.VITE_APP_NAME || "Website"} Admin`}
                      </p>
                    )}
                    <p className="text-sm">{msg.text}</p>
                    <p
                      className={`text-xs mt-1 ${msg.sender === "merchant" || msg.sender === "admin" ? "text-white/70" : "text-text-secondary"}`}
                    >
                      {msg.time}
                    </p>
                  </div>
                </div>
              ))}
          </div>
          <form
            onSubmit={sendMessage}
            className="p-4 border-t border-border/50 flex items-center gap-3"
          >
            <input
              value={messageText}
              onChange={(e) => setMessageText(e.target.value)}
              disabled={
                selectedChat?.status === "resolved" ||
                selectedConversationMeta?.status === "resolved"
              }
              placeholder={t("merchant.typeMessage")}
              maxLength={MAX_MESSAGE_LENGTH}
              aria-label={t("merchant.typeMessage")}
              className="flex-1 px-4 py-2.5 border border-border rounded-full text-sm outline-none focus:border-primary"
            />
            <button
              type="submit"
              disabled={
                sendMessageMutation.isPending ||
                selectedChat?.status === "resolved" ||
                selectedConversationMeta?.status === "resolved"
              }
              className="w-10 h-10 bg-primary text-white rounded-full flex items-center justify-center hover:bg-primary/90 transition"
              aria-label={t("merchant.sendMessage")}
            >
              <Send className="h-4 w-4" />
            </button>
          </form>
          {(selectedChat?.status === "resolved" ||
            selectedConversationMeta?.status === "resolved") && (
            <p className="px-4 pb-3 text-xs text-danger">
              {t("merchant.conversationResolved")}
            </p>
          )}
        </div>
      ) : (
        <div className="flex-1 hidden sm:flex items-center justify-center text-text-secondary">
          {t("merchant.selectConversation")}
        </div>
      )}
    </div>
  );
}
